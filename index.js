try {
        ...
        if (checkForMissingData(user).isMissing == true) {
          ...
          bcrypt.compare(user.password, finalUser.hashedPassword, (err, isMatch) => {
            if (err) {
              ...
              req.session.user = finalUser
              const accountk = randomString({ length: 16, numeric:true, special:false });
              if (!finalUser.accountKeys) {
                finalUser.accountKeys = [];
              } else {
                const accountKeys = finalUser.accountKeys;
                accountKeys.push(accountk);
                finalUser.accountKeys = accountKeys;
              }
              users.set(finalUser.username, finalUser);
              res.send(JSON.stringify({ finished: true, accountKey: accountk })).status(200);
            } else {
              const errorJSON = {
                message: "Password is incorrect."
              }
  ...
  app.post('/signup', async (req, res) => {
    ...
    if (await emails.get(user.email) !== null) {
      ...
      app.get('/api/messages/:id', async (req, res) => {
       ...
       const user = await users.get(req.session.user)
  
        if (user !== null) {
          if (await user.contacts.includes(id)) {
            const memberString = [id, user.username].sort().join('_');
            const conversation = await messages.get(memberString)
            if (conversation) {
  ...
  app.get('/app', async (req, res) => {
    if (!req.session.user) {
      ...
      if (allServers == null || allServers == undefined) {
       ...
       const popularContacts = await getPopularContacts(userr.username)
      if(popularContacts[0]) {
  res.redirect(`/messages/${popularContacts[0].username}`)
      } else {
  ...
  app.get('/messages/:username', async (req, res) => {
    if (!req.session.user) {
      ...
      const msgs = await messages.getAll()
      res.render('messages', { me: me, messages: messageList, user: user, is_desktop, allMessages: msgs, getScore: async (usr) => await getScore(usr), getStatus: async (user) => await getStatus(user) })
    }
  })
  ...
  async function getStatus(user2) {
    const user = await users.get(user2);
    let status;
    if (!user.status || user.status === "") {
      const oldDate = new Date(user.lastOnline);
      const newDate = new Date();
      const diffTime = Math.abs(oldDate.getTime() - newDate.getTime());
      const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      if (diffDays < 1) {
        const diffHours = Math.abs(newDate.getHours() - oldDate.getHours());
        if (diffHours < 6) {
          status = 'last online ' + diffHours + ' hours ago';
        } else {
          status = 'last online today';
        }
      } else {
        status = "offline";
      }
  ...
  app.get('/getMessages/:id', async (req, res) => {
    ...
    if (!messageList) {
    console.log('messageList is null or undefined');
  }
  
  if (!Array.isArray(messageList)) {
    console.log('messageList is not an array');
  }
  
  if (messageList instanceof Array && messageList.length === 0) {
    console.log('messageList is an empty array');
  }
  ...
  async function whatever() {
    const msgs = await messages.getAll()
    const messagesList = Object.keys(msgs).filter(k => k.includes('_'))
    const allMessages = []
    for (const message of messagesList) {
      const msgList = await messages.get(message)
  ...
  res.send(JSON.stringify({ worked: true }));
    ...
    app.get('/user/:username/popularContacts', async (req, res) => {
     const user = await users.get(req.params.username);
      const contacts = user.contacts
     const popularContacts = contacts.sort(async (a, b) => {
       const aMessages = await messages.get([a.username, user.username].sort().join('_'))
       const bMessages = await messages.get([b.username, user.username].sort().join('_'))
  
       const aMessageCount = aMessages.length
       const bMessageCount = bMessages.length
  
       return bMessageCount - aMessageCount
     })
      res.send(JSON.stringify(popularContacts));
     
    });
  
    async function getPopularContacts(username) {
  ...
  app.get('/icon', (req, res) => {
    res.sendFile(__dirname + '/eclipse.png')
  })
    ...
    app.get('/logo', (req, res) => {
      res.sendFile(__dirname + '/fulllogo.png')
    })
